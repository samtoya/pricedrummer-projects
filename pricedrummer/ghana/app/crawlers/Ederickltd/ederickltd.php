<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(!empty($html->find('div[class="pagination paging clearfix"] div[class="links"]', 0))){
				$next_text =  $html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -1)->href;
				echo"<br/>";
			...
			for ($x = 1; $x <= $Number_of_Pages; $x++) {
					...
					//====START COLLECTING THE REQUIRED PRODUCT INFORMATION
					//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
					foreach ($html_new->find('div[class="products-block"] div[class="product-block"]') as $div) {
						echo'<br/>rr<br/>';
						echo	$item_url = urldecode(html_entity_decode ($div->find('div[class="product-meta"] h3[class="name"] a', 0)->href));
						echo'<br/>';
						...
						$Item_Model = "";;	
						echo $image_url = str_replace(" ", "%20",$div->find('div[class="image"] div[class="flip"] img[class="front"]', 0)->src);
						echo'<br/>';echo'<br/>';
					...
					//====END OF COLLECTION OF PRODUCT INFORMATION
				} 
	...
	$merchant_name = strtolower('Ederick Limited');
	$opts = getopt('t:');
