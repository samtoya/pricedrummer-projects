<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if($last = $html->find('div[class="pager"] div[class="pages"] a', -1)) {
				$last_page= intval($html->find('div[class="pager"] div[class="pages"] a', -1)->plaintext);
				
				for ($page = 1; $page <= $last_page; $page++) {
					
					$Current_Page_link =  "https://accrashopper.com/product-category/beauty-and-skin-care/page/".$page."/";
					
					$html_new = file_get_html($Current_Page_link);
					
					
					if(count($html_new->find('ul[class*="products]')) < 2){// Its The Main Product Page
						...
						foreach ($html_new->find('ul[class*="products] li[class*="isotope-item]') as $div) {
							echo'<br/>rr<br/>';
							echo	$item_url = $div->find('div[class="desc"] h4 a', 0)->href;
							echo'<br/>';
							echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]',  0)->innertext);
							echo'<br/>';
							echo	$item_title = $div->find('div[class="desc"] h4 a', 0)->plaintext;	 
							$Item_Model = '';	
	...
	$merchant_name = strtolower('Accra Shopper');
