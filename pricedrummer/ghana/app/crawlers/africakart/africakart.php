...
function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
	...
	include('../include/referers.php');
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class*="products-grid]')) < 2){// The Main Product List Container Was Found
				...
				foreach ($html->find('div[class*="products-grid] li[class="item"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h2[class="product-name"] a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "", $div->find('span[class="price"]', 0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h2[class="product-name"] a', 0)->innertext;	 
					echo'<br/>';
					$ItemDetailsHtml = file_get_html($item_url);
					$unique_div = $ItemDetailsHtml->find('div[class*="product-shop] div[class="box-sku"]', 0);
					$children = $unique_div->children; // get an array of children
					foreach ($children AS $child) {
						$child->outertext = ''; // This removes the element, but MAY NOT remove it from the original $Price_Div
					}
					echo $unique_ID = trim($unique_div->innertext);
					echo'<br/>';
					$Item_Model = '';
					echo $image_url = $div->find('a[class="product-image"] img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('div[class="toolbar-bottom"] div[class="pages"] li a[class*="next]', 0)) {
					$next_page_url = $next->href;
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[id="product-addtocart-button"]')) < 1){
...
$merchant_name = strtolower('Africakart');
