<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			foreach ($html->find('div[class*="product-layout]') as $div) {
				...
				echo'<br/>';
				//prepare price
				echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="caption"] p[class="price"]', 0)->innertext);
				echo'<br/>';
				echo	$item_title = $div->find('div[class="caption"] h4 a', 0)->plaintext;	 
				echo'<br/>';
				...
				$sql = sprintf("SELECT * FROM products where product_name='%s' or url='%s' or and merchant_ID='%s' and status <> 'DELETED' LIMIT 1",
	...
	$merchant_name = strtolower('Baahe');
