<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
		 ...
		 $html->load($result['EXE']);
			...
			if(count($html->find('ul[class*="product-list]')) < 2){// Its The Main Product Page
				...
				echo $html->find('div[class*="list-product-row]',0)->innertext;
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('ul[class*="product-list]',0)->children() as $div) {
					echo "here";
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h5[class="product-name"] a', 0)->href;
					echo'<br/>';
					
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="content_price"] span[class="price"] span[class="amount"]',  0)->innertext);
					
					echo'<br/>';
					echo	$item_title = $div->find('h5[class="product-name"] a', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('img[class="attachment-shop_catalog wp-post-image"]', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('div[class*="bottom-pagination-content] ul[class="pagination"] li[class="pagination_next"] a', 0)) {
					echo $next_page_url = 'http://www.frankotrading.com'.$next->href;
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[class="single_add_to_cart_button button alt"]')) < 1){
	...
	$merchant_name = strtolower('Click Me Fashion World');
	$opts = getopt('t:');
