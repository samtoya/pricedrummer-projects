	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="category-products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="category-products"] ul[class*="products-grid] li[class*="item]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h2[class="product-name"] a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "", $div->find('div[class="price-box"] span[class="price"]', -1)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h2[class="product-name"] a', 0)->innertext;	
					$Item_Model = '';	
					echo $image_url = str_replace(" ", "%20",$div->find('a[class="product-image"] img', 0)->src);
					echo'<br/>';echo'<br/>';
...
$merchant_name = strtolower('Compu Ghana');
