<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			//echo $html->find('div[class="product-grid"]',0)->innertext;
			/*
				
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('div[class="product-list"] div') as $div) {
				
				echo'<br/>rr<br/>';
				echo	$item_url = $div->find('div[class="name"] a', 0)->href;
				echo'<br/>';
				
				//prepare price
				if($div->find('div[class="price"] span[class="price-new"]', 0)){
				echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price"] span[class="price-new"]', 0)->innertext);
				}else{
				echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price"]', 0)->innertext);
				}
				echo'<br/>';
				echo	$item_title = $div->find('div[class="name"] a', 0)->plaintext;	 
				echo'<br/>';
				//echo $item_sku = explode(",",$item_title)[1];
				echo'<br/>';
				$Item_Model = '';	
				echo $image_url = str_replace(" ", "%20",$div->find('div[class="image"] a img', 0)->src);
				echo'<br/>';echo'<br/>';
			...
			*/
			...
			/*
				...
				if($html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -2)){
				$next_text =  $html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -2)->innertext;
				if($next_text == ">" || $next_text == "&gt;" || $next_text == "&#62;"){
				
				$next_page_url =  $html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -2)->href;
			...
			*/
			...
			if(!empty($html->find('div[class="pagination"] div[class="links"]', 0))){
				if(!empty($html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -1))){
					echo $next_text = $html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -1)->href;
			...
			for ($x = 1; $x <= $Number_of_Pages; $x++) {
				...
				//echo $html_new->find('div[class="product-list]',0);
				echo"<h1>".$t = sizeof($html_new->find('div[class="product-grid"] div'));
				echo"</h1><hr/>";
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html_new->find('div[class="product-list"] div') as $div) {
				//echo $div;
					
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a', 0)->href;
					echo'<br/>';
					/*
					//prepare price
					if($div->find('div[class="price"] span[class="price-new"]', 0)){
	...
	$merchant_name = strtolower('computersgh');
