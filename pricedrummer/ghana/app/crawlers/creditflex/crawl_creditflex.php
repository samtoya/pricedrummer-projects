<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="product-wrap"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="product-wrap"] div[class="item-bx"]') as $div) {
			
					echo'<br/>rr<br/>';
					echo	$item_url = 'http://www.creditflexgh.com/'.$div->find('div[class="item-info"] a', 0)->href;
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="item-info"] a h3',  0)->innertext);
					echo'<br/>';
					//prepare title
					$title_Div = $div->find('div[class="item-info"] a h2',  0);
					$children = $title_Div->children; // get an array of children
					foreach ($children AS $child) {
						$child->outertext = ''; // This removes the element, but MAY NOT remove it from the original $Price_Div
					}
					echo	$item_title = $title_Div->innertext;	 
					$Item_Model = '';
					$item_sku = $div->find('div[class="item-info"] a h2 span', 0)->innertext;
					echo $image_url = 'http://www.creditflexgh.com/'.$div->find('div[class="item-img-bx"] a img', 0)->src;
					echo'<br/>';echo'<br/>';
					...
					$sql = sprintf("SELECT * FROM products where product_name='%s' or url='%s' or unique_ID='%s' and merchant_ID='%s' and status <> 'DELETED' LIMIT 1",
				...
				if($next = $html->find('div[class="pagination-box"]', -1)->find('div[class="pagination"] a', -1)->find('span[class="paging"]', 0)) {
					echo $next_page_url = 'http://www.creditflexgh.com/'.$next->parent()->href;
	...
	$merchant_name = strtolower('CreditFlex');
