<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="products-grid"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="products-grid"] div[class*="item]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('div[class="product-info"] div[class="product-name"] a', 0)->href;
					echo'<br/>';
					//prepare price
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price-box"] span[class="price"]', 0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('div[class="product-info"] div[class="product-name"] a', 0)->plaintext;	 
					echo'<br/>';
					$Item_Model = '';	
					echo $image_url = $div->find('div[class="product-image"] a[class="product-image"] img', 0)->src;
					echo'<br/>';echo'<br/>';
	...
	$merchant_name = strtolower('Cyber Mall Ghana');
