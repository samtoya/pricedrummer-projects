<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="items"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="items"] div[class*="product]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h4[class="title"] a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h4[class="title"] a', 0)->innertext;	 
					$Item_Model = "";;	
					echo $image_url = $div->find('a[class="thumb"]', 0)->find('img', 0)->src;
					echo'<br/>';echo'<br/>';
