<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class="products-grid  columns4"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class="products-grid  columns4"] li[class*="item]') as $div) {
					echo'<br/>rr<br/>';
				...
				//NO CHECK FOR NEXT PAGE, USING LIMIT FILTER ON THEIR SITE TO REMOVE PAGINATION
			...
			if($retry_count<$retry_max){
				...
				get_items($url,$category_id,$merchantID,$crawl_url_ID);
	...
	$merchant_name = strtolower('Electrocity');
	$opts = getopt('t:');
