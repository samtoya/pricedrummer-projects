<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[id="product_list"]')) < 2){// The Main Product List Container Was Found
				...
				foreach ($html->find('div[id="product_list"] div[class*="item]') as $div) {
					echo'<br/>rr<br/>';
					$item_url = $div->find('h3[class="title"] a', 0)->href;
					echo'<br/>';
					$price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price new"]', 0)->innertext);
					echo'<br/>';
					$item_title = $div->find('h3[class="title"] a', 0)->innertext;	 
					
					$image_url = $div->find('div[class="preview"] a img[class="product-img1"]', 0)->src;
					$image_url_pieces = explode("/", $image_url);
					$imageName =  $image_url_pieces[count($image_url_pieces)-1];
					//copy($image_url, 'img/'.$imageName);	//this would download the item images into a local folder
					
					
					//split the item title and get the model number from the brackets.
					preg_match_all("/\[([^\]]*)\]/", $item_title, $matches);
					echo '<br/>';
					$Item_Model = $matches[1][0];
					...
					$sql = sprintf("SELECT * FROM products where product_name like '%s' or url like '%s' or model_number like '%s' and merchant_ID='%s' and status <> 'DELETED' LIMIT 1",
					...
					if ($result->num_rows > 0) {
						...
						$sql = sprintf("UPDATE products SET status='%s' where url like '%s' ",
				...
				if($next = $html->find('li[id="pagination_next"] a', 0)) {
					$next_page_url = 'http://www.electrolandgh.com'.$next->href;
	...
	$merchant_name = strtolower('ElecTroland');
