<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[id="load_more_product"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[id="load_more_product"] div[class="products"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = 'http://www.electromart.com/'.$div->find('h3[class="name"] a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "",$div->find('div[class="product-price"] span[class="price"]',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h3[class="name"] a', 0)->innertext;	 
					$Item_Model = "";;	
					echo $image_url = $div->find('div[class="product-image"] div[class="image"] a', 0)->find('img', 0)->src;
					echo'<br/>';echo'<br/>';
