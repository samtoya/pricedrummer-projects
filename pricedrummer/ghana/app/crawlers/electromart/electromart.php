<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="category-products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="category-products"] ul[class*="products-grid] li') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h2[class="product-name"] a', 0)->href;
					echo'<br/>';
					if(count($div->find('span[class="price"]')) >1){
						echo	$price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]', 1)->innertext);
					}else{
					...
					$Item_Model = "";;	
					echo $image_url = $div->find('a[class="product-image"]', 0)->find('img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('a[class="next i-next"]', 0)) {
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[class="button btn-cart"]')) < 1){
	...
	$merchant_name = strtolower('Electromart');
$opts = getopt('t:');
