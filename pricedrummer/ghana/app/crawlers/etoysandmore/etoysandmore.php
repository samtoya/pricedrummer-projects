<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class*="products-list]')) < 2){// Its The Main Product Page
			//die($url);	//SET URL STATUS TO ACTIVE
				...
				foreach ($html->find('ul[class*="products-list] li[class*="product]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a[class="cart-pro-btn"]', 0)->href;
					if (!preg_match('/http:\/\/etoysandmore.com/',$item_url)) {
						$item_url = 'http://etoysandmore.com'.$item_url;
					}
					
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="woocommerce-Price-amount amount"]',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h3', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('a img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('nav[class="pagination woo-pagination"] a[class="next page-numbers"]', 0)) {
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('a[class="button wc-forward"]')) < 1){
	...
	$merchant_name = strtolower('Etoys&more');
	$opts = getopt('t:');
