<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class*="product_list]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class*="product_list] li[class*="ajax_block_product]') as $div) {
					echo "here";
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a[class="product-name"]', 0)->href;
					echo'<br/>';
					
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="content_price"] span[class*="price]',  0)->innertext);
					
					echo'<br/>';
					echo	$item_title = $div->find('a[class="product-name"]', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('div[class="product-image-container"] img', 0)->src;
					echo'<br/>';echo'<br/>';
	...
	$merchant_name = strtolower('Franko Trading');
