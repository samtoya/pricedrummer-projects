<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class="products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class="products"] li') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a[class="entry-title"]', 0)->href;
					echo'<br/>';
					//prepare price
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="entry-meta"] span[class="amount"]', 0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('a[class="entry-title"] h3', 0)->plaintext;	 
					echo'<br/>';
					$Item_Model = '';
					/*
					echo $image_url_style = $div->find('div[class="image"]', 0)->style;
					echo'<br/>';
					preg_match('/\(([^)]+)\)/', $image_url_style, $match);
	...
	$merchant_name = strtolower('hasby');
