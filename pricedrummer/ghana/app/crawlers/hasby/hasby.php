<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			//NO CHECK FOR PRODUCT PAGE
			//echo $html->find('div[class*="product-layout]',0);
			//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
			foreach ($html->find('ol[id="products-list"] li[class*="item]') as $div) {
				echo'<br/>rr<br/>';
				echo	$item_url = $div->find('h2[class="product-name"] a', 0)->href;
				echo'<br/>';
				//prepare price
				if(count($div->find('p[class="special-price"]')) >0){
				echo	$price = preg_replace("/[^0-9.]/", "",$div->find('p[class="special-price"] span[class="price"]', 0)->innertext);
				}else{
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]', 0)->innertext);
				}
				echo'<br/>';
				echo	$item_title = $div->find('h2[class="product-name"] a', 0)->plaintext;	 
				echo'<br/>';
				echo $item_sku = "";
				echo'<br/>';
				$Item_Model = '';	
			...
			//PAGINATION WAS REMOVED WITH THE LIMIT FILTER IN THE URL
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[class="price_btn_buy"]')) < 1){
