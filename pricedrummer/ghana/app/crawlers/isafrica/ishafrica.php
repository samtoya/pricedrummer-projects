<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
		 ...
		 $html->load($result['EXE']);
			...
			if(count($html->find('ul[class="products"]')) < 2){// Its The Main Product Page
				...
				echo count($html->find('div[class="vc_row wpb_row vc_row-fluid"] li'));
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('div[class="vc_row wpb_row vc_row-fluid"] li') as $div) {
					echo "here";
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h3[class="product-name"] a', 0)->href;
					echo'<br/>';
					
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"] span[class="woocommerce-Price-amount amount"]',  0)->innertext);
					
					echo'<br/>';
					echo	$item_title = $div->find('h3[class="product-name"] a', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('img[class*="attachment-shop_catalog]', 0)->src;
					echo'<br/>';echo'<br/>';
	...
	$merchant_name = strtolower('Ishafrica');
