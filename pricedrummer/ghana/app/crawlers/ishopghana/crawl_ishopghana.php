<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		$crawler_product_sleep_seconds = 20;
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="yt-products-container clearfix"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="yt-products-container clearfix"] div[class="item col-lg-3 col-md-4 col-sm-4 respl-item"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('div[class="item-info"] div[class="item-title"] a', 0)->href;
					echo'<br/>';
					//prepare price
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price-box"] span[class="price"]', 0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('div[class="item-info"] div[class="item-title"] a', 0)->plaintext;	 
					echo'<br/>';
					$Item_Model = '';	
					echo $image_url = str_replace(" ", "%20",$div->find('div[class="item-image"] a[class="product-image"] img', 0)->src);
					echo'<br/>';echo'<br/>';
					...
					//sleep($crawler_product_sleep_seconds);
				}
				
				
				//CHECK IF THERE IS A NEXT PAGE OR PAGINATION.
				//IF SO, LOAD THE URL AND COLLECT THE ITEMS.
				if($next = $html->find('div[class="toolbar-bottom"] div[class="pages"] ol li a[class*="next]', 0)) {
	...
	$merchant_name = strtolower('iSHOP Ghana');
