<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[id="catalog-listing"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[id="catalog-listing"] li[class="item isotope-item"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h6[class="item-title"] a', 0)->href;
					echo'<br/>';
					//prepare price
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]', 0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h6[class="item-title"] a', 0)->plaintext;	 
					echo'<br/>';
					$Item_Model = '';	
					echo $image_url = str_replace(" ", "%20",$div->find('div[class="item-images"] img[class="item-image"]', 0)->src);
					echo'<br/>';echo'<br/>';
