<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if($last = $html->find('div[class="v-center justify-end small-auto"] a', -1)) {   //===============Page Is Paginated==============
				$last_page= intval($html->find('div[class="v-center justify-end small-auto"] a', -1)->plaintext);
				
				for ($page = 1; $page <= $last_page; $page++) {
					
					echo $Current_Page_link =  $url."?page=".$page;
					echo"<br/>";
					$html_new = file_get_html($Current_Page_link);
					
					
					//if(count($html_new->find('div[class="row pvn row-products"]')) < 2){ // Its The Main Product Page
						...
						foreach ($html_new->find('div[class*="pvn] div[class="small-3 pvs productItem no-shrink"]') as $div) {
							echo'<br/>rr<br/>';
							echo	$item_url = "https://market.jumia.com.gh".$div->find('a[class="card-overlay block pr"]', 0)->href;
							echo'<br/>';
							echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class*="price]',  0)->innertext);
							echo'<br/>';
							echo	$item_title = $div->find('h3[class="ellipsis phm"]', 0)->plaintext;	 
							$Item_Model = '';	
							echo $image_url = $div->find('div[class="img vh-center"] img', 0)->src;
							echo'<br/>';echo'<br/>';
					...
					//if(count($html_new->find('div[class="row pvn row-products"]')) < 2){ // Its The Main Product Page
						...
						foreach ($html_new->find('div[class*="pvn] div[class="small-3 pvs productItem no-shrink"]') as $div) {
							...
							echo	$item_url = "https://market.jumia.com.gh".$div->find('a[class="card-overlay block pr"]', 0)->href;
							echo'<br/>';
							 $price = preg_replace("/[^0-9.]/", "",$div->find('span[class*="price]',  0)->innertext);
							//echo'<br/>';
							$item_title = $div->find('h3[class="ellipsis phm"]', 0)->plaintext;	 
							$Item_Model = '';	
							$image_url = $div->find('div[class="img vh-center"] img', 0)->src;
							//echo'<br/>';echo'<br/>';
		...
		$merchant_name = strtolower('Jumia Market');
