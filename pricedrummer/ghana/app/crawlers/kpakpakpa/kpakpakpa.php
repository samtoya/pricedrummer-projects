<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			foreach ($html->find('table[width="25%"]') as $div) {
				// echo $div;
				// echo'<br/><br/>';
				
			
				echo'<br/>rr<br/>';
				//derive the base url grom the url sent from the database because this site does not append the base url
				$Base_Category_Url = trim(substr($url, 0, strrpos($url, "/")));
				
				echo $item_url = $Base_Category_Url."/".$div->find('table tr', 0)->find('td p a',0)->href;
				
				
				echo'<br/>';
				//echo $div->find('table tr', 0)->find('td',0)->find('td',0)->innertext;
				
				//echo $div->find('table tr', 0)->find('td p a img',0)->src;
				
				echo $price = preg_replace("/[^0-9.]/", "",$div->find('table tr', 0)->find('td',0)->find('table tr',0)->find('span',0)->innertext);
				echo'<br/>';
				
				echo $item_title = $div->find('table tr', 0)->find('td',0)->find('table tr',1)->find('td',0)->innertext;	 
				
				$Item_Model = "";	
				echo $image_url = str_replace(' ', '%20',$div->find('table tr', 0)->find('td p a img',0)->src);
				echo'<br/>';echo'<br/>';
		...
		if(empty($result['ERR'])){
			...
			$html->load($result['EXE']);
			//die($url);
			if(count($html->find('input[id="button"]')) < 1){
	...
	$merchant_name = strtolower('Kpakpakpa Market');
	$opts = getopt('t:');
