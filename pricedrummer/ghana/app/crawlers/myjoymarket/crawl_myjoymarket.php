<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="index"]')) < 2){// The Main Product List Container Was Found
				...
				foreach ($html->find('div[class="index"] div[class="ad listing"]') as $div) {
					echo'<br/>rr<br/>';
					echo $item_url = "http://www.myjoymarket.com".$div->find('div[class="ad-title"]',0)->find('a',0)->href;
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="ad-price"]', 0)->innertext);
					echo'<br/>';
					echo "Title=>". $item_title = $div->find('div[class="ad-title"]',0)->find('a',0)->plaintext;
					echo'<br/>';
					...
					$item_year = "";
					/*
					$eles = $div->find('ul[class*="ad_detail] li');
					foreach($eles as $e) {
						if(strpos($e->find('span',0)->innertext, 'Year') !== false) {

							echo "Year=>". $item_year = $e->find('b',0)->innertext;	 
							echo'<br/>';
					...
					*/
					...
					if(count($div->find('div[class="ad-image"]')) > 0){
						echo $image_url = "http://www.myjoymarket.com".$div->find('div[class="ad-image"] a img', 0)->src;
					...
					echo	$image_url = "http://www.myjoymarket.com/img/userfiles_resized/small/small_no-image.jpg";
					...
					$sql = sprintf("SELECT * FROM products where product_name like '%s' or url like '%s' and merchant_ID='%s' and status <> 'DELETED' LIMIT 1",
				...
				//SET CRAWLED_URL STATUS TO 1 if done
				...
				if($next = $html->find('div[class="pagination pagination_top"]', 0)->find('a[class="next"]', 0)) {
					$next_page_url ="http://www.myjoymarket.com". urldecode(html_entity_decode ($next->href));
		...
		$merchant_name = strtolower('MyJoyMarket');
		...
		if ($merchant_result->num_rows > 0) {
			...
			//select the urls co crawl for that merchant
				$sql = sprintf("SELECT crawl_url.*, `category`.`crawl_image` FROM crawl_url left join category on crawl_url.category_ID = category.category_ID where merchant_ID='%s' and crawled_status = 0",
					mysqli_real_escape_string($conn,$merchantID));
				$result = $conn->query($sql);
				if ($result->num_rows > 0) {
