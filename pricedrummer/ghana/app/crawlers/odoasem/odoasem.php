<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="box-product"]')) < 2){// Its The Main Product Page
			...
			foreach ($html->find('div[class="box-product"] div[class="prod_wrap"]') as $div) {
				echo'<br/>rr<br/>';
				echo	$item_url = $div->find('div[class="prod_name"] a', 0)->href;
				echo'<br/>';
				//prepare price
				if(count($div->find('div[class="prod_info"] div[class="pricetag2"]')) >0){
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="prod_info"] div[class="pricetag2"]', 0)->innertext);
				}else{
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="prod_info"] div[class="pricetag"]', 0)->innertext);
				}
				echo'<br/>';
				echo	$item_title = $div->find('div[class="prod_name"] a', 0)->plaintext;	 
				echo'<br/>';
				$Item_Model = '';	
				echo $image_url = str_replace(" ", "%20",$div->find('a[class="prod_snimka"] img', 0)->src);
				echo'<br/>';echo'<br/>';
			...
			if($next_text =  $html->find('div[class="pagination"] div[class="links"]', 0)->find('a', -2)->innertext){
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('a[id="button-cart"]')) < 1){
	...
	$merchant_name = strtolower('Odo Asem');
