<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('table[id="offers_table"]')) < 2){// The Main Product List Container Was Found
				...
				foreach ($html->find('table[id="offers_table"] tr') as $div) {
					//echo "theo";
					echo'<br/>rr<br/>';
					//echo $div;
					if(count($div->find('a[class="marginright5 link linkWithHash detailsLink"]')) < 1){
						continue;
					}
					//echo $item_url = $div->find('td[class="offer "] table', 0)->find('tr td', 1)->find('div[class="space rel"] h3 a', 0)->href;
					echo $item_url = $div->find('a[class="marginright5 link linkWithHash detailsLink"]', 0)->href;
					
					
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('p[class="price"] strong', 0)->innertext);
					echo'<br/>';
					echo $item_title = $div->find('a[class="marginright5 link linkWithHash detailsLink"]', 0)->plaintext;	 
					echo'<br/>';

					if(count($div->find('img[class="fleft"]')) > 0){
						
						$image_url = urldecode(html_entity_decode ($div->find('img[class="fleft"]',0)->src));
						...
						$image_url = "https://static.olx.com.gh/packed/img/2fad85f88fe728106bace802813a8d610e.png";
				...
				if($next = $html->find('span[class*="next] a[class*="pageNextPrev]', 0)) {
