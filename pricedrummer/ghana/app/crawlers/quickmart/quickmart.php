<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			foreach ($html->find('div[class="product-list"] div') as $div) {
				...
				echo'<br/>';
				//prepare price
				if(count($div->find('div[class="price"] span[class="price-new"]')) >0){
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price"] span[class="price-new"]', 0)->innertext);
				}else{
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price"] span[class="price-tax"]', 0)->innertext);
				}
	...
	$merchant_name = strtolower('QwickMART');
