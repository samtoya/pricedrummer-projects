<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class*="posts]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class*="posts] div[class="large-3 medium-4 columns end animated fadeIn"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = 'http://slayafrica.storefoundry.com'.$div->find('h5 a[class="title ellipsis"]', 0)->href;
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="disc"] p',  0)->innertext);
					echo'<br/>';
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('input[id="btn_add_to_cart"]')) < 1){
	...
	$merchant_name = strtolower('Slay Africa');
