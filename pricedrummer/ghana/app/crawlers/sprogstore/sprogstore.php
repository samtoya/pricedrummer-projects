<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			//die($url);
			//echo $html->find('div[class="row products-loop products-grid row-count-3"]',0);
			if(count($html->find('div[class*="products-grid]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class*="products-grid] div[class="content-product"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('p[class="product-title"] a', 0)->href;
					echo'<br/>';
					
					if(count($div->find('span[class="price"] span[class="woocommerce-Price-amount amount"]')) >0){
						echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"] span[class="woocommerce-Price-amount amount"]',  0)->innertext);
					}else{
						echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"] ins',  0)->innertext);
					}
					echo'<br/>';
					echo	$item_title = $div->find('p[class="product-title"] a', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = str_replace(" ", "%20",$div->find('a[class="product-content-image"] img', 0)->src);
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('nav[class="pagination-cubic"] ul[class="page-numbers"] li a[class="next page-numbers"]', 0)) {
					echo $next_page_url = $next->href;
		...
		if(empty($result['ERR'])){
			...
			$html->load($result['EXE']);
			//die($html);
			if(count($html->find('button[class*="single_add_to_cart_button]')) < 1){
	...
	$merchant_name = strtolower('Sprog Store');
...
if($opts['t'] ==1){ //////////////////////////////////////////===========================DO RECRAWLING =======================/////////////
	...
	if ($merchant_result->num_rows > 0) {
		...
		while($merchant_row = $merchant_result->fetch_assoc()) {
			...
			echo $merchantID;
			//select the urls co crawl for that merchant
			$sql = sprintf("SELECT * FROM `products` WHERE `merchant_ID` = '%s' and url<>'' ",
				mysqli_real_escape_string($conn,$merchantID));
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
				//collect the data from each url return
