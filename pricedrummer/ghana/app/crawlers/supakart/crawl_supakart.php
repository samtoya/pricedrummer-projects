<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="product-list"]')) < 2){// Its The Main Product Page
			...
			//	echo $html->find('div[class="product-list"]',0)->children()[5]->outertext;
			//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
			foreach ($html->find('div[class="product-list"]',0)->children() as $div) {
				//echo $div;
				echo'<br/>rr<br/>';
				echo	$item_url = $div->find('div[class="name"] a', 0)->href;
				echo'<br/>';
				//prepare price
				echo $price = preg_replace("/[^0-9.]/", "",$div->find('div[class="price"] div[class="boss_price"]', 0)->innertext);
				echo'<br/>';
				echo	$item_title = $div->find('div[class="name"] a', 0)->plaintext;	 
				echo'<br/>';
				$Item_Model = '';	
				echo $image_url = str_replace(" ", "%20",$div->find('div[class="image"] img', 0)->src);
				echo'<br/>';echo'<br/>';
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('input[id="button-cart"]')) < 1){
	...
	$merchant_name = strtolower('SuperKart');
