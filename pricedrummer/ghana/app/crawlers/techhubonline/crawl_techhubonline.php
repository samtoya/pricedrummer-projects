<?php
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class="products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class="products"] li[class*="product]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a[class="woocommerce-LoopProduct-link"]', 0)->href;
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h3', 0)->plaintext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('span[class="et_shop_image"] img', 0)->src;
					echo'<br/>';echo'<br/>';
	...
	$merchant_name = strtolower('Abbey Tech Hub');
