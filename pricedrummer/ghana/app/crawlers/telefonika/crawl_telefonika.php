<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('ul[class="products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class="products"] li[class*="product]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "",$div->find('a span[class="price"]',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('a', 0)->find('h3', 0)->innertext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('a', 0)->find('img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('nav[class="woocommerce-pagination"] ul[class="page-numbers"] li a[class*="next]', 0)) {
	...
	$merchant_name = strtolower('Telefonika');
