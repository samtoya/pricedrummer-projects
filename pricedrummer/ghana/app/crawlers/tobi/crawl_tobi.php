<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			foreach ($html->find('div[class="entry-content"] figure') as $div) {
				// echo $div;
				// echo'<br/><br/>';
				...
				echo'<br/>';
				$caption = (string)$div->find('figcaption', 0)-> plaintext;
				$caption1 = trim(substr($caption, 0, strrpos($caption, "GHc")));
				$raw_price = str_replace($caption1, '',$caption);
				
				echo $price = preg_replace("/[^0-9.]/", "",$raw_price);
				echo'<br/>';
				
				echo $item_title = $caption1;	 
				
				$Item_Model = "";	
				echo $image_url = str_replace(' ', '%20',$div->find('a img', 0)->src);
				echo'<br/>';echo'<br/>';
	...
	$merchant_name = strtolower('Tobi Ghana');
