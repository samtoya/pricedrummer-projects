<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="entry-content"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class="entry-content"] figure[class="wp-caption alignleft"]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = 'http://www.tobighana.com/necklace/';
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('figcaption[class="wp-caption-text"] p',  0)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h5 a[class="title ellipsis"]', 0)->plaintext;	 
					$Item_Model = '';
					echo $image_url = str_replace(" ", "%20",$div->find('img[class="contain-to-grid animated fadeIn"]', 0)->src);
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('nav[class="woocommerce-pagination"] ul[class="page-numbers"] li a[class="next page-numbers"]', 0)) {
	...
	$merchant_name = strtolower('slayafrica');
