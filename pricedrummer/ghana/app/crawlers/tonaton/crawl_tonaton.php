<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="serp-items"]')) < 2){// The Main Product List Container Was Found
				...
				foreach ($html->find('div[class="serp-items"] div[class=ui-item]') as $div) {
					echo'<br/>rr<br/>';
					$item_url = "http://tonaton.com". $div->find('div[class="item-content"] a[class*="item-title]', 0)->href;
					
					$price = preg_replace("/[^0-9.]/", "",$div->find('p[class="item-info"] strong', 0)->innertext);
					echo'<br/>';
					echo $item_title = $div->find('div[class="item-content"] a[class*="item-title]', 0)->innertext;	 
					echo'<br/>';

					if(count($div->find('div[class="item-thumb"] a img')) > 0){
						$image_url = $div->find('div[class="item-thumb"] a img', 0)->getAttribute('data-srcset');
						$image_url_pieces = explode(",", $image_url);
						$image_1_url =  $image_url_pieces[0];
						$image_1_url =  substr($image_1_url, 0, -3);
						$image_url = urldecode(html_entity_decode ("http://". substr($image_1_url, 2)));
						...
						$image_url = "http://w.tonaton-st.com/dist/img/tonaton/all/logos/header-e1873dc3.png";
				...
				if($next = $html->find('a[class*="pag-next]', 0)) {
					$next_page_url = urldecode(html_entity_decode ("http://tonaton.com".$next->href));
		...
		$merchant_name = strtolower('Tonaton');
