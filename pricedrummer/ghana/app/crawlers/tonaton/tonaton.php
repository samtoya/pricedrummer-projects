<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[class="ui-btn is-list item-contact-phone show-contact gtm-show-number"]')) < 1){
...
if($opts['t'] ==1){ //////////////////////////////////////////===========================DO RECRAWLING =======================/////////////
	...
	if ($merchant_result->num_rows > 0) {
		...
		while($merchant_row = $merchant_result->fetch_assoc()) {
			...
			$sql = sprintf("SELECT crawl_url.*, `category`.`crawl_image` FROM crawl_url left join category on crawl_url.category_ID = category.category_ID where merchant_ID='%s' and crawled_status=5",
				mysqli_real_escape_string($conn,$merchantID));
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
