<?php
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[id="products-grid"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[id="products-grid"] div[class*="product-col]') as $div) {
					echo'<br/>rr<br/>';
					echo	$item_url = $div->find('h3[class="product-name name"] a', 0)->href;
					echo'<br/>';
					echo	$price = preg_replace("/[^0-9.]/", "",$div->find('div[class="product-price price"] span[class="price"]',  -1)->innertext);
					echo'<br/>';
					echo	$item_title = $div->find('h3[class="product-name name"] a', 0)->innertext;	 
					$Item_Model = '';	
					echo $image_url = $div->find('A[class="product-image img"] img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('div[class="pager"] div[class="pages"] li[class="button-next"] a[class="next i-next"]', 0)) {
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('button[class="btn button btn-cart validation-passed"]')) < 1){
...
$merchant_name = strtolower('Zoobashop');
