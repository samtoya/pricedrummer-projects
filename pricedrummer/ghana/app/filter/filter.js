angular.module('PxdmSite.filter', ['ngRoute', 'ui.router']).config(function($stateProvider, $urlRouterProvider) {
...
}).controller('FilterCtrl', ['myService', '$scope', '$rootScope', '$http', '$filter', '$stateParams', '$state', '$timeout', '$location', function(myService, $scope, $rootScope, $http, $filter, $stateParams, $state, $timeout, $location) {
    $('html,body').animate({
        scrollTop: 0
    }, 200);
    myService.async().then(function(Token_data) {
        ...
        $scope.getData = function(pageno) {
            ...
            Page_Api_Url = $rootScope.API_ROOT_URL + "productcompare/?category_id=" + $scope.categoryId + "&limit=" + $scope.NumOfProducts + "&offset=" + $scope.offset + $rootScope.Filter_search_string + $scope.InputSearchFilter_for_api + $scope.PriceFilter;
            // console.log(Page_Api_Url);
            Page_Site_Url = "limit=" + $scope.NumOfProducts + "&offset=" + $scope.offset + $rootScope.Filter_search_string + $scope.InputSearchFilter_for_api + $scope.PriceFilter;
            $http({
                method: 'GET',
                ...
                angular.forEach($scope.compare_products_raw, function(product) {
                    ...
                    if (Number_of_Prices ==1 && isMerchantProduct==1) {
                        ...
                        var compare_url = "/contact_seller/" + product.sc_id + "/" + product.id;
