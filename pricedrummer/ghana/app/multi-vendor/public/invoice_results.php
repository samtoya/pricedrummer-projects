	...
	<div class="row">
		<div class="col-md-12 col-lg-12">
			...
			<div class="col-md-11 col-lg-11 no-margin no-padding">
				<div class="col-md-12 col-lg-12">
					<div class="row">
					<div class="col-md-4 col-lg-4" style="margin-left: -45px;">
							<div class="box box-wrapper cf">
								...
								<div class="box-body">
									...
									<div class="fl-form-wrapper">
										<form method="POST" id="view_invoice_form" action="<?php echo $_SERVER['PHP_SELF']; ?>">
		                                    <div class="input-group">
		                                        <input type="text" data-date-format="yyyy-mm-dd"
		                                               data-date-viewmode="years" id="daterange" name="daterange" class="date-time-range">
										...
										<?php
											if ($retailer_invoice_result->num_rows > 0) {
												while($row = $retailer_invoice_result->fetch_assoc()) {
													?>
													<tr <?php if(isset($row['invoice_type'])&&trim($row['invoice_type'])==="BUDGET_SET"){echo "style='color: green;'";}else if(isset($row['invoice_type'])&&trim($row['invoice_type'])==="ITEM_CLICKED"){ echo "style='color: #f03224;'";}else{echo "";} ?> >
														<td><?php if(isset($row['user_ip'])&&trim($row['user_ip'])==="Total Budget Balance"){echo "<strong>".$row['user_ip']."</strong>";}else{ echo $row['user_ip'];} ?></td>
														<td><?php if(isset($row['item_clicked'])&&trim($row['item_clicked'])!=""){echo "<a href='".$row['item_clicked']."' target='new'>View Product</a>";}else{ echo "";}?>
															
														</td>
														<td><?php echo $row['posted_timestamp']; ?></td>
														<td><?php echo $row['category_name']; ?></td>
														<td><?php echo str_replace("_", " ",$row['invoice_type']); ?></td>
														<td style="text-align: left;"><?php if(isset($row['user_ip'])&&trim($row['user_ip'])==="Total Budget Balance"){echo "<strong>".$row['amount']."</strong>";}else{ echo $row['amount'];} ?> </td>
													</tr>
													<?php
												}
											}
