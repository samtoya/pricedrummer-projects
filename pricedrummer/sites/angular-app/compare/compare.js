...
}).controller('CompareCtrl', ['myService', '$scope', '$rootScope', '$http', '$filter', '$stateParams', '$state', '$uibModal', '$rootScope', 'orderByFilter', '$timeout','$window', function(myService, $scope, $rootScope, $http, $filter, $stateParams, $state, $uibModal, $rootScope, orderBy, $timeout, $window) {
                ...
                if($scope.Merchant_Product_IDs[0].trim() != ''){
                    ...
                    angular.forEach($scope.Merchant_Product_IDs, function(Product_id) {
                        $http({
                            ...
                            $http({
                                method: 'POST',
                                url: 'api/get_current_price.php',
                                data: { 'url': response.data.url,
                                    'merchant': response.data.merchant_id},
                                headers : {'Content-Type': 'application/x-www-form-urlencoded'}
                            }).then(function(response1) {
                                console.log();
                                if(response1.data.trim() != '' && isNaN(parseFloat(response1.data)) == false){
                                    Prod_Retailer_Info['price'] = parseFloat(response1.data);
                                    $timeout(function() {
                                        $scope.compare_product['min_current_price'] = Math.min.apply(Math,$scope.product_retailers.map(function(product){return product.price;}));
                                        console.log( $scope.compare_product['min_current_price']);
                                    });
                                }else{
                ...
                if($scope.Retailer_Product_IDs[0].trim() != ''){
                    ...
                    angular.forEach($scope.Retailer_Product_IDs, function(Retailer_Product_id) {
                        $http({
                            ...
                            Retailer_product_Info['url'] = "/contact_seller/" + $scope.SC_ProductId + "/" + Retailer_Product_id;
