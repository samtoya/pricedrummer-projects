angular.module('PxdmSite.seller', ['ngRoute', 'ui.router'])
    ...
    .config(function ($stateProvider, $urlRouterProvider) {
        $stateProvider.state('/contact_seller/:sc_productId/:retailer_productId', {
            url: '/contact_seller/:sc_productId/:retailer_productId',
    ...
    .controller('ContactSellerCtrl', ['myService', '$scope', '$rootScope', '$http', '$filter', '$stateParams', '$state', '$timeout', '$location', function(myService, $scope, $rootScope, $http, $filter, $stateParams, $state, $timeout, $location) {
        ...
        myService.async().then(function (Token_data) {
            ...
            $scope.retailer_productId = $stateParams.retailer_productId;
            ...
            $http({
                ...
                $http({
                    method: 'GET',
                    url: $rootScope.API_ROOT_URL + 'productcompare/?sc_id=' + $scope.product.sc_ID + '&limit=1'
                }).then(function(response) {
                    $scope.compare_product = response.data.results[0];
            ...
            $http({
                method: 'GET',
                url: $rootScope.API_ROOT_URL + 'retailer_products/' + $scope.retailer_productId + '/'
            }).then(function(response) {
