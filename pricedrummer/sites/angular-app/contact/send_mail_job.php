...
$username   = "root";
$password   = "7936_Pxd$*M";                      //pass for server is PxdM1to9
$dbname     = "pxdm_cr";
...
if ( $email_list_result->num_rows > 0 ) {
	while ( $email_row = $email_list_result->fetch_assoc() ) {
		...
		if ( $mail_result ) {
			echo "Mail Sent Successfully - Mail_id :=> {$mail_id}" . PHP_EOL;
			...
			if ( $Update_Mail_Status_result ) {
				echo "Mail Status Updated successfully" . PHP_EOL;
			} else {
				echo "Mail Status Not Updated! mail_id=> {$mail_id}" . PHP_EOL;
			}
		} else {
			echo "Mail Not Sent-Mail_id :=> {$mail_id}" . PHP_EOL;
		}
	}
}
