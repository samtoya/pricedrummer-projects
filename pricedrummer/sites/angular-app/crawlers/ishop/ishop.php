	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="category-products"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('ul[class*="products-grid] li[class*="item]') as $div) {
					echo'<br/>rr<br/>';
						echo $item_url = $div->find('h2[class="product-name"]a', 0)->href;
						echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('p[class="special-price"] span[class="price"]', 0)->plaintext);


					echo'<br/>';
					echo	$item_title = $div->find('h2[class="product-name"]', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
					echo $image_url = $div->find('div[class"product-image-area"] img', 0)->src;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('div[class="pages"] ol li a[class="next i-next"]', 0)) {
...
$merchant_name = strtolower('ishop');
//echo $merchant_name;
