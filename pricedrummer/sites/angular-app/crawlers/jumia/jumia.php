...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('section[class="products"]')) < 2){// Its The Main Product Page
				...
				//echo  $html->find('ul[class*="products-grid] li[class*="item]', 0);
				
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('section [class="products"] div[class*="sku -gallery]') as $div)  {
					echo'<br/>rr<br/>';

					echo $item_url = $div->find('[class="link"]a', 0)->href;
					echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "", $div->find('span[class="price"]', 0)->last_child()->innertext);


					echo'<br/>';
			        echo $item_title = $div->find('span[class="name"] ', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
		 			 echo $image_url  = $div->find('div[class="image-wrapper default-state"]  img', 0)->attr['data-src'];
		 			//echo $image_url = "https:" . $image;
			...
			if($html->find('ul[class="osh-pagination -horizontal"] li', -1)->find('a',0)->getAttribute('title') =='Next'){
				echo 'I CAN GO TO THE NEXT PAGE';
				if($next = $html->find('ul[class="osh-pagination -horizontal"] li', -1)->find('a',0)) {
				...
				echo '<h1>THAT IS THE LAST PAGE</h1>';
			}
