...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="row products-category"]')) < 2){// Its The Main Product Page
				...
				//echo  $html->find('div[class="row cat_prod"] div[class="product-layout product-grid col-lg-4 col-md-4 col-sm-6 col-xs-12"]',0);
				
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('div[class*="product-layout]') as $div) {
					echo'<br/>rr<br/>';
					echo $item_url = $div->find('div[class="caption"] a', 0)->href;
					echo'<br/>';
					
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price-new"]', 0)->plaintext);
					echo'<br/>';
					echo $item_title = $div->find('div[class="caption"] h4', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
					echo $image_url = $div->find('div[class="image"] img', 0)->src;
					echo'<br/>';echo'<br/>';
			...
			/*	if(is_object($html->find('ul[class="pagination"] li', -2))){
				  $next = $html->find('ul[class="pagination"] li', -2)->find('a', 0)->innertext;

				  				if($next == ">" || $next== "&gt;" || $next== "&#62;"){

						$next_page_url =  $html->find('ul[class="pagination"] li', -2)->find('a', 0);
						//$next_page_url =  $html->find('ul[class="pagination"] li', -2)->find('a', 0)->href
					echo $next_page_url = $next_page_url->href;
...
$merchant_name = strtolower('lo2taonline');
//echo $merchant_name;
