...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="row cat_prod"]')) < 2){// Its The Main Product Page
				...
				foreach ($html->find('div[class*="product-layout]') as $div) {
					...
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('p[class="price"]', 0)->plaintext);
					echo'<br/>';
					echo $item_title = $div->find('div[class="caption"] h4', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
					echo $image_url = "http://www.mezashop.com/image/catalog/Logos/meza_logo.png";
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('ul[class="pagination"] li', -2)->find('a', 0)->innertext){
					if($next == ">" || $next== "&gt;" || $next== "&#62;"){
...
$merchant_name = strtolower('mezashop');
//echo $merchant_name;
