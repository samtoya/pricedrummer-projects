...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="products-grid the-list-ftw"]')) < 2){// Its The Main Product Page
			...
			foreach ($html->find('div[class="row"] div[class="item col-lg-3 col-md-3 col-sm-6 col-xs-12"]') as $div) {
					echo'<br/>rr<br/>';
						echo $item_url = $div->find('div[class="product-name"]a', 0)->href;
						echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price"]', 0)->plaintext);


					echo'<br/>';
					echo	$item_title = $div->find('div[class="product-name"]', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
				...
				if($next = $html->find('div[class="pagination"] ol li a[class="next"]', 0)) {
...
$merchant_name = strtolower('rayashop');
//echo $merchant_name;
