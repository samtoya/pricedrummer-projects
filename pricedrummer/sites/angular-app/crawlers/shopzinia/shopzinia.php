...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="product_list grid row"]')) < 2){// Its The Main Product Page
				...
				//echo  $html->find('ol[class="row products-grid"] li[class="col-lg-4 col-md-4 col-sm-4 col-xs-6 item"]', 0);
				
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('ul[class="product_list grid row"] li[class*="ajax_block_product]') as $div) {
					echo'<br/>rr<br/>';
						echo $item_url = $div->find('h5[class="product-name-container"]a', 0)->href;
						echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="price product-price"]', 0)->plaintext);


					echo'<br/>';
			        echo $item_title = $div->find('h5[class="product-name-container"]', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
					echo $image_url = "https://static.shopzinia.com/img/shopziniacom-logo-1456254681.jpg";
			...
			/*	if($next = $html->find('ul[class="pagination"] li[class="pagination_next"]a', 0)) {
					echo $next_page_url = $next->href;
					$next_page_url = "https://shopzinia.com" . $next_page_url;
		...
		if(empty($result['ERR'])){
			...
			if(count($html->find('a[class="buy buybtn"]')) < 1){
...
$merchant_name = strtolower('shopzinia');
//echo $merchant_name;
