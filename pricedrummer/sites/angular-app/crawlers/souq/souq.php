...
error_reporting(E_ALL);
	...
	function get_items($url,$category_id,$merchantID,$crawl_url_ID,$crawl_image){	
		...
		if (empty($result['ERR'])) {//there was no erros with request
			...
			if(count($html->find('div[class="grid-list"]')) < 2){// Its The Main Product Page
				...
				//echo  $html->find('ul[class*="products]', 0);
				
				//LOOP THROUGH THE ITEMS FOUND ON THE PAGE AND COLLECT THE VITAL DETAILS NEEDED.
				foreach ($html->find('div[class="row collapse content medium-up-3"] div[class="column column-block block-grid-large single-item"]') as $div) {
					echo'<br/>rr<br/>';
						echo $item_url = $div->find('h6[class="title"]a', 0)->href;
						echo'<br/>';
					echo $price = preg_replace("/[^0-9.]/", "",$div->find('span[class="is block sk-clr1"]', 0)->plaintext);


					echo'<br/>';
					echo $item_title = $div->find('h6[class="title"]', 0)->plaintext;	
					echo'<br/>'; 
					$Item_Model = '';	
					echo $image_url  = $div->find('div[class="img-bucket"]img', 0)->src;
					 // echo $image_url = "http:" .  $image;
					echo'<br/>';echo'<br/>';
				...
				if($next = $html->find('nav[class="woocommerce-pagination"] a[class="next page-numbers"]', 0)) {
					echo $next_page_url = $next->href;
					//$next_page_url = "http://www.zando.co.za" . $next_page_url;
