<?php
	...
	$delivery_cost = $conn->real_escape_string($_POST['delivery_cost']);
	...
	$add_product_sql = "INSERT INTO `retailer_products` (`retailer_id`, `price`, `manufacturer`, `model_nuber`, `category`, `graphical_url`, `manufacturer_sku`, `has_delevery`, `availability`, `description`)VALUES 
	(".$retailer_Id.", '".$price."', '".$manufacturer."', '".$model_name."', '".$category_id."', '".$image_url."', '".$sku."', '".$has_delivery."', '".$availability."', '".$description."')";
...
//=============IMAGES=========================
	...
	if(!empty($Image_1_name)){
		...
		// Image submitted by form. Open it
			...
			$product_image_sql = "INSERT INTO `retailer_product_images` (`retailer_id`, `image`) VALUES (".$Retailer_product_id.", '".$content."')";
	...
	if(!empty($other_image1_name)){
		if(($other_image1_extension == 'jpg' || $other_image1_extension == 'jpeg' || $other_image1_extension == 'png') && ($other_image1_type == 'image/jpeg' || $other_image1_type == 'image/png')){
			...
			$other_image1_content = base64_encode($other_image1_image);
			$other_image1_content= $conn->real_escape_string($other_image1_content);
			$other_image1_name= $conn->real_escape_string($other_image1_name);

			$other_image1_sql = "INSERT INTO `retailer_product_images` (`retailer_id`, `image`) VALUES (".$Retailer_product_id.", '".$other_image1_content."')";
	...
	if(!empty($other_image2_name)){
		if(($other_image2_extension == 'jpg' || $other_image2_extension == 'jpeg' || $other_image2_extension == 'png') && ($other_image2_type == 'image/jpeg' || $other_image2_type == 'image/png')){
			...
			$other_image2_content = base64_encode($other_image2_image);
			$other_image2_content= $conn->real_escape_string($other_image2_content);
			$other_image2_name= $conn->real_escape_string($other_image2_name);

			$other_image2_sql = "INSERT INTO `retailer_product_images` (`retailer_id`, `image`) VALUES (".$Retailer_product_id.", '".$other_image2_content."')";
	...
	if(!empty($other_image3_name)){
		if(($other_image3_extension == 'jpg' || $other_image3_extension == 'jpeg' || $other_image3_extension == 'png') && ($other_image3_type == 'image/jpeg' || $other_image3_type == 'image/png')){
			...
			$other_image3_content = base64_encode($other_image3_image);
			$other_image3_content= $conn->real_escape_string($other_image3_content);
			$other_image3_name= $conn->real_escape_string($other_image3_name);

			$other_image3_sql = "INSERT INTO `retailer_product_images` (`retailer_id`, `image`) VALUES (".$Retailer_product_id.", '".$other_image3_content."')";
