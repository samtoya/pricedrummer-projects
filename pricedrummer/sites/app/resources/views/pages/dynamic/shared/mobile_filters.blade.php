<div class="shop-sidebar">
            ...
            @foreach($MatchingCategories as $category)
                <p style="margin: 0;">
                    <a onclick="closeCanvas();"
                       href="/filter/{{$category->category_id}}/search?search=<?php if ( isset( $_GET[ 'search' ] ) ) {
                           echo $_GET[ 'search' ];
                       }?>&view={{$view}}"
    ...
    @if(count($RelatedCategories) > 0)
        <div style="font-size: 13px;margin: 0;padding: 10px 0 8px 0;border-bottom: #2197C9 3px solid;">
            ...
            @foreach($RelatedCategories as $category)
                <p style="margin: 0;">
                    <a onclick="closeCanvas();"
                       href="/filter/{{$category->category_id}}/{{lowercase(spacelessUrl($category->name))}}"
                       style="color: #2197C9; margin: 8px 10px;">
                        {{$category->name}}
                    </a>
                </p>
    ...
    <div class="coupon-accordion">
        ...
        @foreach($category_details as $category_code => $category_values)
            <div>
                <h3 class="show filter-heading" onclick="ToggleFilterSpecs('m{{ReformatID($category_code)}}', 'mhead-icon')">
                ...
                <div id="m{{ReformatID($category_code)}}" class="coupon-checkout<?php if ( $filter_count > 1 ) {
                    echo '-content';
                }?>">
                                    ...
                                    <div class="checkbox checkbox-primary">
                                        ...
                                        @if( strpos($category_code, '|range') !== false )
                                            ...
                                            <div>{{--range slider display values--}}
                                                <span id="m{{explode("|", $category_code)[0]}}-range-min"></span>
                                                <span class="pull-right"
                                                      id="m{{explode("|", $category_code)[0]}}-range-max"></span>
                                            </div>
                                            {{-- the range slider itself --}}
                                            <div id="m{{explode("|", $category_code)[0]}}-range"></div>
                                            ...
                                            <script type="text/javascript">
                                                $(document).ready(function () {
                                                    init_range_slider(
                                                        "m{{explode("|", $category_code)[0]}}-range",
                                                            {{min($category_values)}},
                                                            {{max($category_values)}},
                                                    ...
                                                    @else
                                                        "{{''}}"
                                                            @endif,
                                                        "m{{explode("|", $category_code)[0]}}-range-min",
                                                        "m{{explode("|", $category_code)[0]}}-range-max",
                                                    ...
                                                    <script type="text/javascript">
                                                        $(document).ready(function () {
                                                            generate_filter_element_url(
                                                                "m{{explode("|", $category_code)[0].$key}}",
                                                                "m{{explode("|", $category_code)[0]}}",
                                                                "{{explode("|", $category_code)[0]}}",
                                                                "m",
