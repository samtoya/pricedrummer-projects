<?php
	...
	if(isset($_GET['pid'])){
		$product_id = urldecode($_GET['pid']);
		$sql = 'SELECT products.*,(select image from crawled_images c where c.product_ID=products.product_ID limit 1) as image FROM products  WHERE `products`.product_ID ='.$product_id;
	...
	if(isset($_GET['rpid'])){
		$product_id = urldecode($_GET['rpid']);
		$sql = 'SELECT retailer_products.*,CONCAT(`retailer_products`.manufacturer," ", `retailer_products`.model_nuber) As product_name,`retailer_products`.model_nuber as model_number,"" as url, `retailer_products`.id as product_ID,(select image from retailer_product_images c where c.retailer_product_id=retailer_products.id limit 1) as image FROM retailer_products  WHERE `retailer_products`.id ='.$product_id;
		$result = $conn->query($sql);
		if($result){
			$row_product = $result->fetch_assoc();
		}
	...
	$Cat_sql = 'SELECT * FROM category WHERE category_ID ='.$row_product['category'];
	...
	$Cat_Specs_sql = 'SELECT * FROM category_details WHERE category_ID ='.$row_product['category'];
				...
				<div class="main-content">
					...
					<div class="container">
						...
						<div class="row">
							...
							<form enctype="multipart/form-data" class="form-horizontal" method="post" action="include/addtosc.php" id="Product_Check_Form"> 
								<!-- START MODAL-->
											...
											<div class="modal-body">
												<p>
													<div class="form-group">
														...
														<select id="lev1" name="Level1" class="form-control" onchange="get_cat_lev_2(this); change_specs($(this).val()); $('#L1display').html($(this).find('option:selected').text()); $('#Drill_Level').val(1); " style="height: 33px;">
															<option value="">&nbsp;</option>
															<?php
											...
											<div class="modal-body">
												<p>
													<div class="form-group">
														...
														<select id="lev2" name="Level2" class="form-control" onchange="get_cat_lev_3(this); change_specs($(this).val()); $('#L2display').html($(this).find('option:selected').text());  $('#Drill_Level').val(2);" style="height: 33px;">
															<option value="">&nbsp;</option>
														</select>
													</div>
												</p>
											...
											<div class="modal-body">
												<p>
													<div class="form-group">
														...
														<select id="lev3" name="Level3" class="form-control" onchange="get_cat_lev_4(this); change_specs($(this).val()); $('#L3display').html($(this).find('option:selected').text()); $('#Drill_Level').val(3);" style="height: 33px;">
															<option value="">&nbsp;</option>
														</select>
													</div>
												</p>
											...
											<div class="modal-body">
												<p>
													<div class="form-group">
														...
														<select id="lev4" name="Level4" class="form-control" onchange="change_specs($(this).val());$('#L4display').html($(this).find('option:selected').text()); $('#Drill_Level').val(4);" style="height: 33px;">
															<option value="">&nbsp;</option>
														</select>
													</div>
												</p>
								...
								<div class = "col-md-6">
									<div class="panel panel-danger">
										<div class="panel-heading">
											<h3 class="panel-title"> Standard Catalogue</h3>
											<a href="<?php echo $row_product['url']; ?>" class="btn btn-xs btn-red tooltips pull-right" target="blank" style="width:40px; height: 25px; margin-top: -20px;"data-placement="top" data-original-title="View Product on merchant site"><i class="fa fa-link" style="padding-top: 5px;"></i></a>
										</div>
										
										<input type="hidden" id="Drill_Level" name="Cat_level" value="0"/>
										<?php if(isset($_GET['pid'])){?>
											<input type="hidden" id="MerchantProductID" name="MerchantProductID" value="<?php echo $_GET['pid']; ?>"/>
										...
										<?php  }else{ ?>
											<input type="hidden" id="RetailerProductID" name="RetailerProductID" value="<?php echo $_GET['rpid']; ?>"/>
										<?php } ?>
										...
										<table class = "table table-bordered" >
											...
											<thead>
												<tr>
													<th width="10%" bgcolor="#F2DEDE" style="color:black;">Specs Section.</th>
													<th width="25%" bgcolor="#F2DEDE" style="color:black;">Item Specs.</th>
											...
											<tbody id="Specs_Table">	
												<tr>
													...
													<td>Model No.</td>
													<td>
														<input type="text" alt="" value="<?php echo $row_product['model_number']; ?>" name="MODEL_NUMBER" id="product_model_Number" onkeyup="$('#check_model').show();">
														<button type="button" id="check_model" class="btn btn-light-orange pull-right">CHECK</button>
													</td>
												</tr>
												...
												<tr>
													...
													<td><input style="width: 100%; color: black;" type="text" name="ALT_NAME1" value="" id="product_alt_name1"></td>
												</tr>
												<tr>
													<td>Standard</td>
													<td>Alternative Name 2</td>
													<td><input style="width: 100%; color: black;" type="text" name="ALT_NAME2" value="" id="product_alt_name2"></td>
												</tr>
												<tr>
													<td><b><font color="red">MERCHANT</font></b></td>
													<td>Product Name</td>
													<td><input style="width: 100%; color: black;" type="text" value="<?php echo $row_product['product_name']; ?>" id="product_name"></td>
												</tr>
												<tr>
													<td><b><font color="red">MERCHANT</font></b></td>
													<td>Product Image</td>
													<td>
														...
														<?php if(!empty($row_product["image"])){
															echo '<a class="thumb-info" href="data:image;base64,'.$row_product["image"].' " data-lightbox="gallery" data-title="'.$row_product["product_name"].'">
															<img src="data:image;base64,'.$row_product["image"].' " class="img-responsive" alt="" height="50" width="50">
															<span class="thumb-info-title"></span>
															</a>';
														?>
														
														<?php if(isset($_GET['pid'])){?>
															<label for="Add_Crawed_Image">Check to add this Image</label>
															<input type="checkbox" id="Add_Crawed_Image" name="Add_Crawed_Image" value="Yes" />
														<?php } ?>

														<?php if(isset($_GET['rpid'])){?>
															<label for="Add_Retailer_Image">Check to add this Image</label>
															<input type="checkbox" id="Add_Retailer_Image" name="Add_Retailer_Image" value="Yes" />
														<?php } ?>
													...
													if ($Cat_Specs_result->num_rows > 0) {
														...
														<tr  class="Specs_tr">
															...
															<?php 
																...
																?>
																<td><input style="width: 100%; color: black;" name="ItemSpecs[<?php echo  $Cat_Specs_row['details_name']."|".$Cat_Specs_row['details_code'] ?>]" type="text" value="N/A" onkeydown="<?php if(isset($Cat_Specs_row['NumCheck']) && $Cat_Specs_row['NumCheck'] =="1"){echo 'numberOnly(this);';} ?>" onfocus="this.select();<?php if(isset($Cat_Specs_row['NumCheck']) && $Cat_Specs_row['NumCheck'] =="1"){echo 'numberOnly(this);';} ?>" id="<?php
																	//if the detail_name contain space, then replace the space with _ before using it as id.
																	//this is to aid id targeting it from jquery
																if(preg_match('/\s/',$Cat_Specs_row['details_name']) > 0){ echo preg_replace('/\s+/', '_', $Cat_Specs_row['details_name']);}else{ echo $Cat_Specs_row['details_name'];} ?>" alt="<?php if(isset($Cat_Specs_row['suggestion']) && $Cat_Specs_row['suggestion'] !=''){echo  $Cat_Specs_row['suggestion'];}?>">
										...
										<div class="panel-footer">	
											<div>
												<?php if(isset($row_product['Description']) && !empty($row_product['Description'])){
					...
					<script>
						...
						set_levels(<?php echo $row_product['category']; ?>);
						...
						$(document).ready(function(){
							$('#check_model').click(function(){
								var exist_modID = $('#product_model_Number').val().trim();
									...
									if(success_msg == 1){
										...
										$.ajax({
											...
											success:function(data2){
												str = data2.replace(/\s+/g, '');//Remove Any Space
										...
										$.ajax({
											url: 'include/collect_specs_for_change.php',
											type: 'POST',
											data: {cat_id:cat_id},
											success:function(data){
												$(".Specs_tr").remove();
