...
//	ini_set('max_execution_time', 0);
	...
	if ($Product_Images_result->num_rows > 0) {
		while($Product_Images_row = $Product_Images_result->fetch_assoc()) {
			...
			//$path = "img";
			$path = "../../static/product_images/db_images";
			$path_for_compression=  "../../static/product_images/";
			...
			$path_to_uncompressed_file = $path . '/' . $Product_Images_row['image_ID'].'.png';
			
			// Uploads the RAW file.
			$path_to_raw_file = $raw_uploads_dir . $Product_Images_row['image_ID'].'.png';
			
			// Uploads the compressed
			$path_to_compressed_file = $large_uploads_dir . $Product_Images_row['image_ID'].'.png';
			...
			if ( file_exists( $path_to_compressed_file ) ) {
				...
				$resize = new \phMagick\Action\Resize\Proportional( $image_to_resize, $medium_uploads_dir . $Product_Images_row['image_ID'].'.png');
				...
				$resize = new \phMagick\Action\Resize\Proportional( $image_to_resize, $small_uploads_dir . $Product_Images_row['image_ID'].'.png');
				...
				$resize = new \phMagick\Action\Resize\Proportional( $image_to_resize, $thumbs_uploads_dir . $Product_Images_row['image_ID'].'.png');
