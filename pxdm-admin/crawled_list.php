...
if(isset($_GET['Search'])){
	...
	$Search = $conn->real_escape_string(urldecode($_GET['Search']));
	// find out how many rows are in the table
	$sql_count = 'SELECT * FROM `products` WHERE `product_name` LIKE "%'.$Search.'%" OR `url` LIKE "%'.$Search.'%" OR `price` LIKE "%'.$Search.'%" and status <> "DELETED" and sc_status = "IN_SC"';
	if($result9){
	...
	$sql = 'SELECT  `products`.*,`category`.`name`, (select image from sc_images where product_ID= sc_ID limit 1)as image FROM `products` left join `category` on products.`category` = `category`.`category_ID` WHERE `product_name` LIKE "%'.$Search.'%" OR `url` LIKE "%'.$Search.'%" OR `price` LIKE "%'.$Search.'%" and status <> "DELETED" and sc_status = "IN_SC" LIMIT '.$offset.', '.$rowsperpage;
	...
	if(isset($_GET['Uncategorised'])){

		// Set session variables
		$_SESSION["Category_Status"] = "Uncategorised";
		$Category_Status = $_SESSION["Category_Status"];

		// find out how many rows are in the table
		$sql_count = "SELECT count(*) as numrows FROM `uncategorised_products_view` WHERE category IN(".$child_categories_ids.")";
		...
		// get the info from the db
		$sql = "SELECT * FROM `uncategorised_products_view` WHERE category IN(".$child_categories_ids.") LIMIT ".$offset.", ".$rowsperpage;
//		die($sql);
		$result = $conn->query($sql);
	...
	if(isset($_GET['Categorised'])){
		...
		// find out how many rows are in the table
		$sql_count = "SELECT count(*) as numrows FROM `categorised_products_view` WHERE category IN(".$child_categories_ids.")";
		...
		// get the info from the db
		$sql = "SELECT * FROM `categorised_products_view` WHERE category IN(".$child_categories_ids.") LIMIT ".$offset.", ".$rowsperpage;
		$result = $conn->query($sql);
	...
	if(isset($_GET['In_SC'])){
		...
		// Set session variables
		$_SESSION["Category_Status"] = "In_SC";
		$Category_Status = $_SESSION["Category_Status"];

		// find out how many rows are in the table
		$sql_count = 'SELECT count(*) as numrows FROM `in_sc_products_view` WHERE category IN('.$child_categories_ids.')';
		$result9 = $conn->query($sql_count);
		if($result9){
            $data = $result9->fetch_assoc();
            $numrows = $data['numrows'];
		...
		// get the info from the db
		$sql = "SELECT * FROM `in_sc_products_view` WHERE category IN(".$child_categories_ids.") LIMIT ".$offset.", ".$rowsperpage;
		$result = $conn->query($sql);
	...
	if(isset($_GET['Tagged'])){
		...
		// find out how many rows are in the table
		$sql_count = 'SELECT count(*) as numrows FROM `tagged_products_view` WHERE category IN('.$child_categories_ids.')';
		$result9 = $conn->query($sql_count);
		if($result9){
            $data = $result9->fetch_assoc();
            $numrows = $data['numrows'];
		...
		// get the info from the db
		$sql = "SELECT * FROM `tagged_products_view` WHERE category IN(".$child_categories_ids.") LIMIT ".$offset.", ".$rowsperpage;
		$result = $conn->query($sql);
									...
									if ($result->num_rows > 0) {
										while($row = $result->fetch_assoc()) {
											...
											<tr class="">
												...
												<td ondblclick="StartPriceEdit(this)" style="cursor:pointer;">
													<label for="price" class="control-label"  >
														<p class="text-info" ><?php echo $row["price"];?></p>
													</label>
													<input type="text" class="price-edit-input" style="display:none;width: 100px;" value="<?php echo $row['price'];?>" onblur="ProcessPriceChange(this)" />
												...
												<td class="center">
													<div class="visible-md visible-lg hidden-sm hidden-xs">
														<?php if(isset($row["sc_status"]) && $row["sc_status"] == 'ACCEPTED' and !isset($_GET['Search'])){?>

															<a href="#" class="btn btn-xs btn-blue tooltips" style="width:40px; height: 25px;"data-placement="top" data-original-title="Reason Flag" data-target=".ReasonCode-modal-sm" data-toggle="modal" onclick="$('#P_to_tag_ID').val('<?php echo $row["product_ID"];?>'); set_row(this)"><i class="fa  fa-flag-checkered"></i></a>

															<a href="check_sc.php?pid=<?php echo urlencode($row["product_ID"]);?>" class="btn btn-xs btn-green tooltips" style="width:40px; height: 25px;" onclick=""><i class="fa fa-save"></i></a>


														<?php }elseif(isset($row["sc_status"]) && $row["sc_status"] == 'NEW' and !isset($_GET['Search'])){

															?>

															<a href="#" class="btn btn-xs btn-blue tooltips" style="width:40px; height: 25px;"data-placement="top" data-original-title="Reason Flag" data-target=".ReasonCode-modal-sm" data-toggle="modal" onclick="$('#P_to_tag_ID').val('<?php echo $row["product_ID"];?>'); set_row(this)"><i class="fa  fa-flag-checkered"></i></a>

															<a href="#" class="btn btn-xs btn-red tooltips" style="width:40px; height: 25px;"data-placement="top" data-original-title="Categorize" data-target=".bs-example-modal-sm" data-toggle="modal" onclick="set_levels(<?php echo $row["category"];?>); $('#p_ID').val('<?php echo $row["product_ID"];?>'); set_row(this)"><i class="fa fa-puzzle-piece"></i></a>
															<?php
															...
															<a href="#" class="btn btn-xs btn-red tooltips" style="width:40px; height: 25px;"data-placement="top" data-original-title="Categorize" data-target=".bs-example-modal-sm" data-toggle="modal" onclick="set_levels(<?php echo $row["category"];?>); $('#p_ID').val('<?php echo $row["product_ID"];?>'); set_row(this)"><i class="fa fa-edit"></i></a>

															<a href="check_sc.php?pid=<?php echo urlencode($row["product_ID"]);?>" class="btn btn-xs btn-green tooltips" style="width:30px; height: 20px;" onclick=""><i class="fa fa-save"></i></a>


														<?php }else{ ?>


															<a href="#" class="btn btn-xs btn-blue tooltips" style="width:30px; height: 20px;"data-placement="top" data-original-title="Reason Flag" data-target=".ReasonCode-modal-sm" data-toggle="modal" onclick="$('#P_to_tag_ID').val('<?php echo $row["product_ID"];?>'); set_row(this)"><i class="fa  fa-flag-checkered"></i></a>
									...
									<div class=" pull-right">
										...
										if ($currentpage > 1) {
											...
											if(isset($_GET['Search'])){
												...
												echo " <a href='{$_SERVER['PHP_SELF']}?Search=$Search&currentpage=1' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-left fa-lg'></i></a> ";
												// get previous page num
												$prevpage = $currentpage - 1;
												// show < link to go back to 1 page
												echo " <a href='{$_SERVER['PHP_SELF']}?Search=$Search&currentpage=$prevpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-left fa-lg'></i></a> ";

											}else{

												// show << link to go back to page 1
												echo " <a href='{$_SERVER['PHP_SELF']}?$Category_Status=&Category=$Category&Cat_level=$Cat_level&currentpage=1' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-left fa-lg'></i></a> ";
												// get previous page num
												$prevpage = $currentpage - 1;
												// show < link to go back to 1 page
												echo " <a href='{$_SERVER['PHP_SELF']}?$Category_Status=&Category=$Category&Cat_level=$Cat_level&currentpage=$prevpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-left fa-lg'></i></a> ";
											}
										...
										for ($x = ($currentpage - $range); $x < (($currentpage + $range)  + 1); $x++) {
											...
											if (($x > 0) && ($x <= $totalpages)) {
												...
												if ($x == $currentpage) {
													...
													if(isset($_GET['Search'])){
														// make it a link
														echo " <a href='{$_SERVER['PHP_SELF']}?Search=$Search&currentpage=$x' style='width: 30px; height: 30px; padding: 5px;' class='btn btn-primary'>$x</a> ";
													}else{
														// make it a link
														echo " <a href='{$_SERVER['PHP_SELF']}?$Category_Status=&Category=$Category&Cat_level=$Cat_level&currentpage=$x' style='width: 30px; height: 30px; padding: 5px;' class='btn btn-primary'>$x</a> ";
													}
										...
										if ($currentpage != $totalpages) {
											...
											if(isset($_GET['Search'])){
												echo " <a href='{$_SERVER['PHP_SELF']}?Search=$Search&currentpage=$nextpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-right fa-lg'></i></a> ";

												// echo forward link for lastpage
												echo " <a href='{$_SERVER['PHP_SELF']}?Search=$Search&currentpage=$totalpages' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-right fa-lg'></i></a> ";
											}else{
												echo " <a href='{$_SERVER['PHP_SELF']}?$Category_Status=&Category=$Category&Cat_level=$Cat_level&currentpage=$nextpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-right fa-lg'></i></a> ";

												// echo forward link for lastpage
												echo " <a href='{$_SERVER['PHP_SELF']}?$Category_Status=&Category=$Category&Cat_level=$Cat_level&currentpage=$totalpages' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-right fa-lg'></i></a> ";
											}
...
<script>
	...
	$("#UpdateLev").click(function() {
		...
		//alert(p_ID);
		$.ajax({
			url: 'include/update_product_category.php',
			type: 'POST',
