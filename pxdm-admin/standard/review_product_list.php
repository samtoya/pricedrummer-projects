...
$sql_count = 'SELECT`sc`.*,(select image from sc_images where product_ID=sc_ID limit 1) as image,`category`.`name`,`category`.`category_ID` FROM `sc` left join `category` on `sc`.`category_ID` = `category`.`category_ID`  WHERE `sc`.`status` ="RECHECK" and `sc`.`status` <>"DELETED" AND `sc`.`category_ID` IN('.$child_categories_ids.')';
...
$sql = 'SELECT`sc`.*,(select image from sc_images where product_ID=sc_ID limit 1) as image,`category`.`name`,`category`.`category_ID` FROM `sc` left join `category` on `sc`.`category_ID` = `category`.`category_ID`  WHERE `sc`.`status` ="RECHECK" and `sc`.`status` <>"DELETED" AND `sc`.`category_ID` IN('.$child_categories_ids.')LIMIT '.$offset.', '.$rowsperpage;
	$result = $conn->query($sql);
												...
												<?php
													if ($result->num_rows > 0) {
														...
														<tr class="">
															...
															<td class="center">
															<a href="edit_sc_product.php?Cat=<?php echo $row["category_ID"]?>&Prod=<?php echo $row["sc_ID"]?>" class="btn btn-xs btn-green tooltips" style="width:40px; height: 25px;"><i class="fa fa-edit" style="padding-top: 5px;"></i></a></td>
														</tr>
									...
									<div class="col-md-6">
										<div class=" pull-right">
											...
											if ($currentpage > 1) {
												...
												echo " <a href='{$_SERVER['PHP_SELF']}?Category=$Category&Cat_level=$Cat_level&currentpage=1' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-left fa-lg'></i></a> ";
												// get previous page num
												$prevpage = $currentpage - 1;
												// show < link to go back to 1 page
												echo " <a href='{$_SERVER['PHP_SELF']}?Category=$Category&Cat_level=$Cat_level&currentpage=$prevpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-left fa-lg'></i></a> ";
											} // end if
											...
											for ($x = ($currentpage - $range); $x < (($currentpage + $range)  + 1); $x++) {
												...
												if (($x > 0) && ($x <= $totalpages)) {
													...
													if ($x == $currentpage) {
														...
														// make it a link
														echo " <a href='{$_SERVER['PHP_SELF']}?Category=$Category&Cat_level=$Cat_level&currentpage=$x' style='width: 30px; height: 30px; padding: 5px;' class='btn btn-primary'>$x</a> ";
													} // end else
												} // end if
											} // end for
											...
											if ($currentpage != $totalpages) {
												...
												echo " <a href='{$_SERVER['PHP_SELF']}?$Category=$Category&Cat_level=$Cat_level&currentpage=$nextpage' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-primary'><i class='fa fa-angle-right fa-lg'></i></a> ";

												// echo forward link for lastpage
												echo " <a href='{$_SERVER['PHP_SELF']}?Category=$Category&Cat_level=$Cat_level&currentpage=$totalpages' style='width: 30px; height: 30px; padding: 5px; padding-top: 7px;' class='btn btn-default'><i class='fa fa-angle-double-right fa-lg'></i></a> ";
											} // end if
